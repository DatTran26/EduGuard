...
export function buildExamFormValues(exam, defaultClassroomId = "") {
  return {
    ...
    settings: {
      ...
      requireCamera: Boolean(exam?.settings?.requireCamera),
      requireMicrophone: Boolean(exam?.settings?.requireMicrophone),
      enableLiveProctoring: Boolean(exam?.settings?.enableLiveProctoring),
      enableCameraProctoring: Boolean(exam?.settings?.enableCameraProctoring),
      enableExternalDeviceDetection: Boolean(exam?.settings?.enableExternalDeviceDetection),
      captureSnapshotOnViolation: Boolean(exam?.settings?.captureSnapshotOnViolation),
      enableRealtimeWarning: exam?.settings?.enableRealtimeWarning ?? true,
    },
...
export function buildSubmitPayload(formValues) {
  return {
    ...
    settings: {
      ...
      requireCamera: Boolean(formValues.settings.requireCamera),
      requireMicrophone: Boolean(formValues.settings.requireMicrophone),
      enableLiveProctoring: Boolean(formValues.settings.enableLiveProctoring),
      enableCameraProctoring: Boolean(formValues.settings.enableCameraProctoring),
      enableExternalDeviceDetection: Boolean(formValues.settings.enableExternalDeviceDetection),
      captureSnapshotOnViolation: Boolean(formValues.settings.captureSnapshotOnViolation),
      enableRealtimeWarning: formValues.settings.enableRealtimeWarning !== false,
    },
