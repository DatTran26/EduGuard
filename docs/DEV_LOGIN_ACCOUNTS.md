...
## Khắc phục khi không đăng nhập được

1. **Connection string SQL** — file `backend/EduGuard.Api/appsettings.Development.json` phải trỏ đúng instance trên máy bạn (vd. `TRANDAT\SQLEXPRESS` hoặc `localhost\SQLEXPRESS`). Nếu sai, API trả lỗi 500 và form login có thể hiển thị nhầm là sai mật khẩu.
2. **Seed lại tài khoản** (reset mật khẩu về `Test@12345`):

   ```powershell
   dotnet run --project backend\scripts\SeedDevUsers
   ```

3. **Khởi động lại API** sau khi sửa connection string hoặc seed.
4. **Swagger** — thử `POST /api/auth/login` trước; nếu Swagger OK mà web vẫn lỗi thì kiểm tra frontend proxy (`http://127.0.0.1:5157`).
