...
**Cập nhật:** 2026-06-27 (tái cấu trúc giao diện giám sát thi của admin (AdminMonitoringPage) loại bỏ KPI Điểm nghi ngờ, làm nổi bật và thu nhỏ chiều cao 3 KPI còn lại bằng 3 màu sắc khác nhau để giảm khoảng trống, bo tròn tab, bỏ số lượng bản ghi và cách nhau bởi dấu "|", giới hạn tối đa 10 dòng cuộn dọc, thêm modal tổng hợp danh sách tất cả học sinh khi click vào KPI tổng sinh viên trên dashboard giảng viên, chuyển đổi form tạo bài tập/bài thi của giáo viên sang dạng modal căn giữa làm mờ nền và tự động ẩn khi lưu xong, tinh chỉnh 6 card KPI trên dashboard giảng viên hài hòa chế độ sáng với nền pastel tương thích cao, thiết kế lại 6 card KPI trên dashboard giảng viên sặc sỡ hơn theo tone màu và thêm micro-animations, sửa breadcrumb chi tiết bài kiểm tra hiển thị tên đề thật, đổi tab `Bài tập` của Teacher trên `/teacher/tasks` sang flow bảng danh sách -> workspace chấm bài riêng, thêm nhãn `Đã chấm/Chưa chấm`, đưa bài tập mới tạo lên đầu danh sách, và làm lại hierarchy hiển thị cho card bài thi trên nhánh `devH`)
**Ghi chú devH:** 2026-06-27 (thiết kế lại giao diện Giám sát thi hệ thống (AdminMonitoringPage) loại bỏ KPI Điểm nghi ngờ, làm nổi bật và thu gọn chiều cao 3 KPI (Tổng log, Lượt cảnh báo, Nguy cơ cao) với border-l, tone nền tương ứng và padding nhỏ hơn; bo tròn tab dạng rounded-full không hiển thị số lượng và phân tách bằng dấu "|"; bộ lọc được chuyển thành nút icon bên phải mở ra panel tùy chọn; bọc danh sách bằng container cuộn giới hạn tối đa 10 dòng; click vào card KPI Tổng sinh viên ở Dashboard giảng viên sẽ mở modal hiển thị danh sách tất cả sinh viên từ các lớp học do giảng viên đó quản lý (lấy lớp + thành viên đồng thời, loại trùng lặp, hiển thị lớp học tham gia và hỗ trợ bộ lọc tìm kiếm); chuyển đổi form tạo bài tập và bài thi ở trang hoạt động học tập `/teacher/tasks` sang dạng modal hội thoại hiển thị chính giữa màn hình với nền xung quanh bị mờ và nhòe (`backdrop-blur-[2px]` `bg-slate-900/60`) để tập trung cao độ, tích hợp nút đóng `FiX` và đóng khi click ngoài, tự động ẩn/đóng modal khi tạo thành công (xóa tham số `create` trên URL); tinh chỉnh 6 card KPI trên Dashboard giảng viên để tối ưu hiển thị chuyên nghiệp với chế độ sáng: chuyển nền từ xám đồng nhất sang màu trắng tinh tế (`bg-white` / `dark:bg-slate-900`) kết hợp viền xám nhẹ (`border-slate-200/80` / `dark:border-slate-800/80`) để tránh bị xỉn màu; nâng tương phản chữ của nhãn và số liệu; mở rộng padding thẻ thành `p-[18px]` và bo tròn góc `rounded-2xl` mềm mại; tăng kích thước icon wrapper lên `h-9 w-9` và icon lên `h-4.5 w-4.5` xoay nhẹ khi hover; căn trái nội dung toàn thẻ để tạo trục đứng gọn gàng; breadcrumb ở trang chi tiết bài kiểm tra của Teacher/Student nay resolve tên đề thật thay vì hiện `examId`...)
...
**Ghi chú devH:** 2026-06-26 (sửa lỗi trắng màn hình (crashed UI do ReferenceError) tại trang chi tiết lớp học ClassroomDetailPage.jsx bằng cách import bổ sung hằng hằng số định hướng TEACHER_CLASSROOM_TABS bị thiếu.)
...
**Ghi chú devB:** 2026-06-26 (đã sửa kiểm tra đủ câu ma trận để so khớp chương/bài/yêu cầu cần đạt theo kiểu trim + không phân biệt hoa thường, không loại các câu hỏi cũ đang trống môn trong ngân hàng đã chọn, và hiển thị thêm `Môn` trong điều kiện đủ/thiếu để người dùng thấy đầy đủ bộ lọc.)
**Ghi chú devB:** 2026-06-26 (đã chặn lỗi tạo đề từ ma trận trả 500 chung chung bằng cách validate `Đóng đề > Mở đề` ở backend/frontend và dùng mapper cấu hình đề thi đầy đủ khi sinh đề từ ma trận.)
**Ghi chú devB:** 2026-06-26 (đã đổi `Sinh đề từ ma trận` sang workflow B: giáo viên sinh đề nháp từ ma trận, sửa câu trong popup chỉ trên snapshot đề nháp và không cập nhật ngân hàng, hệ thống chỉ cảnh báo nếu đề nháp lệch ma trận, sau đó xác nhận đề nháp để mở popup chọn lớp/tiêu đề/giờ mở đề/cấu hình và tạo bài kiểm tra thật đã publish theo lịch từ đúng đề nháp đã chỉnh.)
**Ghi chú devB:** 2026-06-26 (đã sửa popup đặt lịch của đề từ ma trận để khi chọn `Mở đề` thì tự cộng `Thời gian làm bài` của ma trận vào `Đóng đề`; đồng thời sửa lỗi tạo bài kiểm tra từ ma trận báo 500 do DB local còn pending migration live proctoring: đổi FK `ProctoringEvidences -> CheatingLogs` sang `NO ACTION`, apply `dotnet ef database update`, và thêm catch `DbUpdateException` trả lỗi tiếng Việt thay vì 500 chung chung.)
...
- [x] Bảng quản lí bài tập trong workspace Teacher `/teacher/tasks` *(tab `Bài tập` nay hiển thị bảng gồm tên bài tập, tên lớp, hạn nộp, chưa chấm, đã nộp; thêm 3 icon thao tác để mở chấm, chỉnh sửa, xóa; khi bấm tên bài tập hoặc icon mắt sẽ ẩn bảng và chuyển sang workspace chấm bài riêng có nút quay lại danh sách, còn icon bút mở màn chỉnh sửa riêng không kèm workspace chấm bài; trong danh sách bài nộp có badge `Đã chấm/Chưa chấm` cho từng sinh viên; danh sách bài tập mặc định đưa bài mới tạo lên đầu để dễ tìm)*
...
- [x] Breadcrumb chi tiết đề thi hiển thị tên bài kiểm tra *(TopBar ở route `/teacher/exams/:examId` và `/student/exams/:examId` nay gọi exam API để thay segment số bằng `exam.title`, giúp điều hướng rõ ràng hơn khi mở chi tiết đề)*
- [x] Card bài thi trong workspace Teacher `/teacher/tasks?type=exam` *(hiển thị rõ tên lớp, làm nổi bật tên bài kiểm tra, canh giữa phần class/title/stats, bỏ dòng `Tạo: ...`, bỏ nhãn phụ `Lớp học/Câu hỏi/Lượt làm`, thu card gọn hơn một chút, và đổi lịch mở/đóng đề thành timeline 2 dòng với 1 icon cùng vạch dọc thể hiện khoảng thời gian)*
...
- [x] UI ngân hàng câu hỏi và ma trận đề thi *(route `/teacher/question-banks` mở bằng danh sách ngân hàng trước; bấm ngân hàng mới vào màn chỉnh sửa, form thêm câu hỏi dạng panel ngang có thể thu gọn, nhãn `Yêu cầu cần đạt` thay cho `Chuẩn đầu ra`, nút sửa câu hỏi mở popup riêng để giữ vị trí trong danh sách, list câu hỏi full-width, breadcrumb/dropdown/badge/toast đã Việt hóa có dấu, badge trạng thái/độ khó có dấu nhận diện, nút nhảy sang ma trận, form ma trận nhập `Tổng điểm` và tự tính `Điểm/câu`, tóm tắt realtime theo độ khó, các phần ma trận đã lưu/chi tiết/kiểm tra/tạo đề là card full-width xếp dọc, khối chi tiết ma trận có tổng quan/thống kê/dòng chi tiết/bảng đủ-thiếu câu, popup ma trận nêu rõ đang thiếu điều kiện nào/cần bao nhiêu/có bao nhiêu, và `Sinh đề nháp` chỉ bật sau khi kiểm tra đủ câu thành công)*
